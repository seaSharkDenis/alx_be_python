task = input("Enter you task description: ")
priority= input("Enter your task's prioriyt (high, medium, low): ").lower()
time_bound = input("Is the task time-bound? (yes or no) ").lower()

match priority:
    case "high":
        if time_bound == "yes":
            print(f"Reminder: '{task}' is a high priority tasks that required immediate attention today!")
        else:
    ...
    case "low":
        if time_bound == "yes":
            print(f"Reminder: '{task}' is a low priority task. It however required you to allocate some time for it")
        else:
